...
class ConversationRepository(
    ...
    suspend fun getConversationDetails(conversationId: String): Result<Conversation?> {
        ...
        return try {
            ...
            if (documentSnapshot.exists()) {
                ...
                Log.d(
                    "ConvRepo" ,
                    "Conversation details fetched successfully for ID $conversationId: $conversation"
                )
        ...
        callbackFlow {
            ...
            val listenerRegistration = messagesCollection.addSnapshotListener { snapshots , error ->
                if (error != null) {
                    Log.e(
                        "ConvRepo" ,
                        "Firestore listen failed for messages in $conversationId." ,
                        error
                    )
                ...
                if (snapshots != null) {
                    val messagesList = snapshots.documents.mapNotNull { doc ->
                        try {
                            ...
                            Log.e(
                                "ConvRepo" ,
                                "Error parsing message document ${doc.id} in $conversationId" ,
                                e
                            )
                            null
                        }
                    ...
                    Log.d(
                        "ConvRepo" ,
                        "Successfully fetched/updated ${messagesList.size} messages for $conversationId."
                    )
                    trySend(Result.success(messagesList))
                } else {
                    Log.d(
                        "ConvRepo" ,
                        "Messages snapshot is null for $conversationId (no error, but no data)."
                    )
    ...
    suspend fun sendTextMessage(
        ...
        if (conversationId.isBlank() || text.isBlank() || senderId.isBlank()) {
            Log.w(
                "ConvRepo" ,
                "sendTextMessage called with invalid parameters. ConvID: $conversationId, Text: '$text', SenderID: $senderId"
            )
        ...
        return try {
            ...
            Log.d(
                "ConvRepo" ,
                "Text message successfully sent to $conversationId. Message ID: ${messageDocumentRef.id}"
            )
    ...
    suspend fun sendOfferMessage(
        ...
        if (conversationId.isBlank() || senderId.isBlank()) {
            Log.w(
                "ConvRepo" ,
                "sendOfferMessage called with invalid parameters. ConvID: $conversationId, SenderID: $senderId"
            )
        ...
        return try {
            Log.d(
                "ConvRepo" ,
                "Attempting to send offer message to $conversationId. Price: $proposedPrice, Text: '$optionalText'"
            )
            val messageDocumentRef = firestore.collection("conversations")
                .document(conversationId)
                .collection("messages")
            ...
            Log.d(
                "ConvRepo" ,
                "Offer message successfully sent to $conversationId. Message ID: ${messageDocumentRef.id}"
            )
            ...
            Log.d(
                "ConvRepo" ,
                "Updated currentNegotiatedPrice to $proposedPrice for $conversationId"
            )
    ...
    private suspend fun updateLastMessage(
        ...
        try {
            ...
            Log.d(
                "ConvRepo" ,
                "Last message updated for conversation $conversationId: '$messageText'"
            )
        } catch (e: Exception) {
    ...
    suspend fun getConversationsForCurrentUser(): Result<List<Conversation>> {
        ...
        return try {
            ...
            Log.d(
                "ConvRepo" ,
                "Fetched ${querySnapshot.size()} conversations for user $currentUserId"
            )
            ...
            val conversations = querySnapshot.documents.mapNotNull { document ->
                try {
                    ...
                    if (otherUserId != null) {
                        ...
                        try {
                            val userDoc =
                                firestore.collection("Person").document(otherUserId).get().await()
            ...
            Log.d(
                "ConvRepo" ,
                "Successfully processed ${sortedConversations.size} conversations for user $currentUserId"
            )
    ...
    suspend fun sendOfferMessage(
        ...
        return try {
            ...
            updateLastMessage(
                conversationId ,
                "Offre: ${String.format("%.2f" , proposedPrice)}€" ,
                senderId
            )
