...
@Composable
fun MaillotDeBainScreen(navController: NavController, sellViewModel: SellViewModel) {

    var selectedType by sellViewModel.selectedType

    Scaffold(
        topBar = {
            VintedTopBar(title = "Maillots de bain", navController, true)
        }
    ) { innerPadding ->
        LazyColumn(
            modifier = Modifier
