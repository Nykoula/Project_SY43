...
@Composable
fun CombinaisonScreen(navController: NavController , sellViewModel: SellViewModel) {

    var selectedType by sellViewModel.selectedType

    Scaffold(
        topBar = {
            VintedTopBar(title = "Combinaisons et combishorts" , navController , true)
        }
    ) { innerPadding ->
        LazyColumn(
            modifier = Modifier
            ...
            items(
                listOf(
                    "Combinaisons" ,
                    "Combi shorts" ,
                    "Autres combinaisons et combishorts"
                )
            ) { type ->
                Row(
