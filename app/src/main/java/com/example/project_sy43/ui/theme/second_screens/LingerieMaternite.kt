...
@Composable
fun LingerieMaterniteScreen(navController: NavController, sellViewModel: SellViewModel) {

    var selectedType by sellViewModel.selectedType

    Scaffold(
        topBar = {
            VintedTopBar(title = "Sous-vêtements maternité", navController, true)
        }
    ) { innerPadding ->
        LazyColumn(
            modifier = Modifier
            ...
            items(
                listOf(
                    ...
                    "Tenues de nuit maternité",
                    "Soutiens-gorge grossesse & allaitement"
                )
            ) { type ->
                Row(
